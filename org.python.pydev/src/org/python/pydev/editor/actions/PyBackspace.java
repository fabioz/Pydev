 ...
 *
 *	We can:
 *	- go to the indentation from some uncommented previous line (if we only
 *	  have whitespaces in the current line).
 *
...
public class PyBackspace extends PyAction{
	...
	public void run(IAction action) {
		...
		{   
		    ...
		    //System.out.println("lastCharRegion.getOffset(): " +lastCharRegion.getOffset());
//			IRegion cursorRegion = ps.doc.getLineInformationOfOffset(cursorOffset);
			
		    if( cursorOffset == lastCharRegion.getOffset()){
		        //System.out.println("We are in the beggining of the line.");
		        //in this situation, we are in the first character of the line...
		        //so, we have to get the end of the other line and delete it.
		        if (cursorOffset != 0) //we only want to erase if we are not in the first line.
     ...
     */
    private void eraseToPreviousIndentation(PySelection ps, boolean hasOnlyWhitespaces) throws BadLocationException {
        ITextSelection textSelection = ps.getITextSelection();
        int indentation = getPreviousIndentation(ps,textSelection);
        if(indentation == -1){
            ...
            if(hasOnlyWhitespaces){
			    ...
			    //  |    a  (delete to previous indentation - considers cursor position)
			    //so, we have to treat it carefully
                //TODO: use the conditions above and not just erase a single char.
     ...
     * @param ps
     * @param textSelection
     * @return offset of the indentation on the previous non-commented line or -1
     *         if we are not able to get it (if this happens, we delete 1 char).
     * @throws BadLocationException
     */
    private int getPreviousIndentation(PySelection ps, ITextSelection textSelection) throws BadLocationException {
        int currentLine = textSelection.getStartLine();
        if(currentLine == 0){
            //we are in the first line, so, we have no basis to get indentation.
            return -1;
        }else{
	...
	private void eraseLineDelimiter(PySelection ps) throws BadLocationException{
	    ...
	    //System.out.println("Replacing offset: "+(offset) +" lenght: "+ (length));
    ...
    /**
     * TODO: Make use of the indentation gotten previously. This implementation just
     * uses the indentation string and erases the number of chars from it.
     * 
     * @param ps
