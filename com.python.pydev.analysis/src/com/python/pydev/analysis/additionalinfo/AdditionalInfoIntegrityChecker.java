...
public class AdditionalInfoIntegrityChecker implements IPyEditListener{
    ...
    public static void checkIntegrity(IProject project, IProgressMonitor monitor){
        ...
        check(existingModuleNames, modulesManager, buffer);
    }

    private static void check(HashSet<ModulesKey> existingModuleNames, IModulesManager modulesManager, StringBuffer buffer) {
        ...
        UIUtils.showString(buffer.toString());
    ...
    public void onCreateActions(ListResourceBundle resources, final PyEdit edit, IProgressMonitor monitor) {
        edit.addOfflineActionListener("--d", new Action(){
            @Override
            public void run() {
                checkIntegrity(edit.getProject(), new NullProgressMonitor());
            }
        });
    }
