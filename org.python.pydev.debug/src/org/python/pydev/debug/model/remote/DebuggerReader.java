...
public class DebuggerReader implements Runnable {
     ...
     * Create it
     * 
     * @param s socketToWrite we are reading from
    ...
    /**
     * keep reading until we finish (that should happen when an exception is thrown, or if it is set as done from outside)
     * 
        ...
        while (!done) {
            try {
                ...
                if (cmdLine != null) {
                    ...
                    //ok, process finished (input stream closed)
                    done = true;
                }
                synchronized (this) {
                    Thread.sleep(50);
