...
public class GeneratePropertiesPage extends UserInputWizardPage {
	...
	public GeneratePropertiesPage(String name, PropertyTreeProvider provider,
			GeneratePropertiesRequestProcessor requestProcessor) {
		super(name);
		this.setTitle(name);
		...
		selectAll.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
				...
				requestProcessor.setCheckedElements(treeViewer
						.getCheckedElements());
				GeneratePropertiesPage.this.getWizard().getContainer()
						.updateButtons();
			}
		});
				...
				.addSelectionChangedListener(new ISelectionChangedListener() {
					public void selectionChanged(SelectionChangedEvent event) {
						...
						if (!sel.isEmpty()) {
							OffsetStrategyModel elem = (OffsetStrategyModel) sel
									.getFirstElement();
							requestProcessor.setPropertyDestination(elem
									.getStrategy());
						}
					}
				});

		requestProcessor.setPropertyDestination(strategyProvider.get(2)
				.getStrategy());
