...
/**
 * This class does depend on PyDev to resolve a module name. The module was
 * resolved (alias etc.) by ModuleAdapter method. Double insertion of
 * ModuleAdapters is detected (represented files are equal)
 * 
 * @author Ueli Kistler
 * 
 */
public class PythonModuleManager {
	...
	public Set<ModuleAdapter> resolveModule(File currentFile,
			FQIdentifier identifier) {

		if (identifier == null || currentFile == null
				|| (!(currentFile.exists()))
				|| identifier.getProbableModuleName().length() == 0)
	...
	private Set<ModuleAdapter> extractModuleAdapter(
	...
	private SortedMap<ModulesKey, ModulesKey> resolveModules(
			FQIdentifier identifier) {
		SortedMap<ModulesKey, ModulesKey> modulesStartingWith = moduleManager
				.getAllModulesStartingWith(identifier.getProbableModuleName());

		if (modulesStartingWith.size() == 0)
			modulesStartingWith = moduleManager
					.getAllModulesStartingWith(identifier.getModule());
		return modulesStartingWith;
	}

	private ModuleAdapter getModuleAdapterFromFile(File file) throws Throwable {
		if (file != null && file.getName().compareTo(INIT) != 0
				&& file.exists()) {
			Document doc = new Document(
					getFileContent(new FileInputStream(file)));
