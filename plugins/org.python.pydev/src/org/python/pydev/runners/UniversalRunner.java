...
public class UniversalRunner {
	...
	public static abstract class AbstractRunner {
		...
		public Tuple<String, String> runCodeAndGetOutput(
				String code, String[] args, File workingDir,
	...
	public static class JythonRunner extends AbstractRunner {
		...
		public String[] getCommandLine(List<String> argumentsAfterPython) {
			try {
				return SimpleJythonRunner.makeExecutableCommandStr(
					nature.getProjectInterpreter().getExecutableOrJar(), 
					argumentsAfterPython.get(0),
					"", 
					argumentsAfterPython.subList(1, argumentsAfterPython.size()).toArray(new String[0])
				);
			} catch (Exception e1) {
	...
	public static class IronPythonRunner extends AbstractRunner {
		...
		public String[] getCommandLine(List<String> argumentsAfterPython) {
			try {
				argumentsAfterPython.add(0, "-u");
				return SimpleIronpythonRunner.preparePythonCallParameters(
						nature.getProjectInterpreter().getExecutableOrJar(), 
						argumentsAfterPython.get(0), 
