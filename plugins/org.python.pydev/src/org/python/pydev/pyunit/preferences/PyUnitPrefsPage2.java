...
public class PyUnitPrefsPage2 extends FieldEditorPreferencePage implements IWorkbenchPreferencePage {
    ...
    protected void createFieldEditors() {
        ...
        tooltipPresenter = new ToolTipPresenterHandler(
            p.getShell(),
            presenter,
            "Tip: Click on the link to add it as a parameter to the test runner.");
        
        addField(createTestRunnerEditor(p));
        parametersField = new MultiStringFieldEditor(
            TEST_RUNNER_DEFAULT_PARAMETERS,
            "Parameters for test runner   (click links below to add flags)",
            p);
        ...
        add(
            "--<a>verbosity</a>=number", 
            "verbosity", 
            "Sets the verbosity level for the run (0-9)\n" +
            " 0: almost no output\n" +
            " 9: many details", 
            p);
        ...
        add(
            ...
            "The --split_jobs flag actually mandates how the tests will be scheduled (if jobs > 1).", 
            p);
        
        add(
            "--<a>split_jobs</a>=tests|module", 
            "split_jobs", 
        ...
        add(
            "--<a>include_files</a>=comma separated list of patterns to match files to include", 
            "include_files", 
        ...
        add(
            "--<a>exclude_files</a>=comma separated list of patterns to match files to exclude", 
            "exclude_files", 
        ...
        add(
            "--<a>include_tests</a>=comma separated list of patterns to match tests to include", 
            "include_tests", 
        ...
        add(
            "--<a>exclude_tests</a>=comma separated list of patterns to match tests to exclude", 
            "exclude_tests", 
    ...
    private void add(String linkText, String flag, String tooltip, Composite p) {
        addField(new LinkFieldEditor("link_"+flag, linkText, p, new PyUnitPrefsPage2.PyUnitPageLinkListener(flag), tooltip+"\n", tooltipPresenter));
    }
