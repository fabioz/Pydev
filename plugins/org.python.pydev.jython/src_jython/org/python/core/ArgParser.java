...
public class ArgParser {
    ...
    public ArgParser(String funcname, PyObject[] args, String[] kws, String[] paramnames, int minargs) {
        ...
        if (!PyBuiltinFunction.DefaultInfo.check(args.length, minargs, this.params.length)) {
            throw PyBuiltinFunction.DefaultInfo.unexpectedCall(args.length, false, funcname, minargs,
                    this.params.length);
        }
    }
        ...
        l1: for (int i = 0; i < this.kws.length; i++) {
            for (int j = 0; j < this.params.length; j++) {
                if (this.kws[i].equals(this.params[j])) {
                    if (j < nargs) {
                        throw Py.TypeError("keyword parameter '" + this.params[j]
    ...
    private Object getArg(int pos, Class clss, String classname, Object def) {
        ...
        if (ret == Py.NoConversion) {
            throw Py.TypeError("argument " + (pos + 1) + ": expected " + classname + ", " + Py.safeRepr(value)
                    + " found");
        }
        return ret;
    }
