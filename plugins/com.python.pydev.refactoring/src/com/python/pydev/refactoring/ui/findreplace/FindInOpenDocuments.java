...
public class FindInOpenDocuments {
	...
	public static void findInOpenDocuments(
    	...
    	if(window == null){
    		if(statusLineManager != null)statusLineManager.setErrorMessage("Active workbench window is null.");
    		return;
    	}
				...
				if(realEditor instanceof MultiPageEditorPart){
    				...
    				if(input != null){
    					...
    					if(file != null){
    						...
    						if(statusLineManager != null)statusLineManager.setMessage("Warning: Editors not in the workspace cannot be searched.");
		...
		if(files.size() == 0){
			if(statusLineManager != null)statusLineManager.setMessage("No file was found to perform the search (editors not in the workspace cannot be searched).");
			return;
		}
