 ...
 * 
 */
public final class RewriterVisitor extends AbstractRewriterVisitor {
...
//    }
	...
	private SimpleNode handleCallArguments(Call node) throws Exception {
		...
		if (printer.getNodeHelper().isFilledList(node.keywords)) {
			// comma before already handled in visitWithSeparator
	...
	private boolean handleCommaOptional(SimpleNode node) {
		return (printer.hasSpecialAfter(node, SyntaxHelper.COMMA));
	}
	...
	private void handleIfElse(If node) throws Exception {
		...
		if (node.orelse != null) {
	        java.util.List<commentType> commentsBefore = new ArrayList<commentType>();
	        java.util.List<commentType> commentsAfter = new ArrayList<commentType>();
	        java.util.List<commentType> before = printer.extractComments(node.orelse.specialsBefore);
	        if(before != null){
	            commentsBefore.addAll(before);
	        }
	        int lastLine = node.orelse.beginLine;
	        java.util.List<commentType> after = printer.extractComments(node.orelse.specialsAfter);
	        if(after != null){
	            for(commentType c:after){
	                if(c.beginLine <= lastLine+1){
	                    commentsBefore.add(c);
	                }else{
	                    commentsAfter.add(c);
	                }
	            }
			...
			for (stmtType statement : node.orelse.body) {
				passedElsif = handleIfElseSuite(node.orelse, passedElsif, statement, commentsBefore);
				commentsBefore=null;
			}
			if (passedElsif) {
				printer.outdent();
			}
			printer.printComment(node, commentsAfter);
		}
	}

	private boolean handleIfElseSuite(suiteType orelse, boolean passedElsif, stmtType statement, java.util.List<commentType> commentsAfterElse) throws Exception {
		...
		if (printer.getNodeHelper().isIfStatement(statement)) {
			...
			if(commentsAfterElse!=null){
			    printer.printComment(orelse, commentsAfterElse);
			}
			printer.indent();
			printer.printNewlineAndIndentation();
		}
	...
	private void handleRootNode(modType node, stmtType[] body) throws Exception {
		...
		SimpleNode last = visit(node, body, false, false);
		if(!(last instanceof FunctionDef)){
		    handleCommentAfter(last);
		}
		handleCommentAfter(node);
	}

	private void handleSeparator(SimpleNode parent, SimpleNode lastNode, boolean hasNext) {
		if (hasNext) {
	...
	private SimpleNode visit(SimpleNode parent, SimpleNode[] list, boolean outdent, boolean separator) throws Exception {
		...
		for (int i = 0; i < list.length; i++) {
			node = list[i];
			boolean hasNext = i != list.length - 1;
			boolean isComprehension = printer.getNodeHelper().isComprehension(parent);
			...
			if (separator) {
				handleSeparator(parent, node, hasNext);
			}

			handleCommentAfter(node);

			if (hasNext) {
				if (!separator && !isComprehension) {
					printer.printNewlineAndIndentation();
				}
			} else {
				if (outdent) {
