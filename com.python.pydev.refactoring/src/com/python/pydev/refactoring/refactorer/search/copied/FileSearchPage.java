...
public class FileSearchPage extends AbstractTextSearchViewPage implements IAdaptable {
	...
	protected void fillContextMenu(IMenuManager mgr) {
		...
		if (query.getSearchString().length() > 0) {
			...
			if (!selection.isEmpty()) {
				ReplaceAction replaceSelection= new ReplaceAction(getSite().getShell(), (PythonFileSearchResult) getInput(), selection.toArray(), true);
			...
			ReplaceAction replaceAll= new ReplaceAction(getSite().getShell(), (PythonFileSearchResult) getInput(), null, true);
	...
	private boolean showLineMatches() {
		...
		return getLayout() == FLAG_LAYOUT_TREE && input != null && !((AbstractPythonSearchQuery) input.getQuery()).isFileNameSearch();
	}
