 ...
 * 
 * Straightforward 'way': - find the definition and assert it is not a global -
 ...
 * case 1:
 * 
 * def m1(): a = 1 def m2(): a = 3 print a print a
 * 
 * case 2:
 * 
 * def m1(): a = 1 def m2(): print a a = 3 print a print a
 * 
 * case 3:
 * 
 * def m1(): a = 1 def m2(): if foo: a = 3 print a print a
 * 
 * if we rename it inside of m2, do we have to rename it in scope m1 too? what
 * about renaming it in m1?
 * 
 * The solution that will be implemented will be:
 *  - if we rename it inside of m2, it will only rename inside of its scope in
 * any case (the problem is when the rename request commes from an 'upper'
 * scope).
 *  - if we rename it inside of m1, it will rename it in m1 and m2 only if it is
 * used inside that scope before an assign this means that it will rename in m2
 * in case 2 and 3, but not in case 1.
 */
